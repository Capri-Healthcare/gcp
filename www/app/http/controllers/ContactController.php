...
class ContactController extends Controller
{
	...
	**/
	...
	public function indexAction()
	{
		...
		if (!isset($_POST['make-request']) || !$validation['status']) {
			$this->session->data['message'] = array('alert' => 'warning', 'value' => $validation['message']); //$lang['text_please_enter_valid_data_in_input_box']);
	...
	protected function validate($data) {
		...
		**/		
		if ((strlen($data['email']) > 96) || !filter_var($data['email'], FILTER_VALIDATE_EMAIL)) {
			...
			* Return false
			**/
			return array('status' => false, 'message' => $lang['text_email_error']);
		} elseif (strlen($data['name']) < 2) {
			/** 
			* If name is not valid (min 2 character)
			* Return false
			**/
			return array('status' => false, 'message' => $lang['text_name_error']);
		} elseif ((strlen($data['mobile']) < 4) || (strlen($data['mobile']) > 32)) {
			/** 
			* If Mobile number is not valid ( min 4 character or max 32 ) 
			* Return false
			**/
			return array('status' => false, 'message' => $lang['text_mobile_error']);
		} elseif (strlen($data['subject']) < 5) {
			/** 
			* If Subject is not valid (min 5 character)
			* Return false
			**/
			return array('status' => false, 'message' => $lang['text_email_subject_error']);
		} elseif (strlen($data['body']) < 10) {
			/** 
			* If Subject is not valid (min 5 character)
			* Return false
			**/
			return array('status' => false, 'message' => $lang['text_email_body_error']);
		} else {
