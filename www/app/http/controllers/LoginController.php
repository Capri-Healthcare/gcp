...
class LoginController extends Controller
{
    ...
    public function login()
    {
        ...
        if ($data['role'] == 0) {
            ...
            if ($user = $this->model_login->checkAdminUser($data)) {
                ...
                if (!$this->checkLoginAttempts($user['email'])) {
                    $this->session->data['message'] = array('alert' => 'error', 'value' => "Warning: Your account has exceeded allowed number of login attempts. Please try again in 1 hour.");
                    $this->url->redirect('login');
                    ...
                    if (password_verify($data['password'], $user['password'])) {
                        ...
                        echo '<script>window.location = "https://lc.gcp.caprihealthcare.co.uk/admin/index.php?route=login"</script>';
                    } else {
                        /**
                         * Add login attemt to Db
                        ...
                        $this->session->data['message'] = array('alert' => 'error', 'value' => "Warning: No match for Username and/or Password.");
                     ...
                     * Then Show error to user
                     **/
                    $this->session->data['message'] = array('alert' => 'error', 'value' => "Warning: Your account has disabled for more info contact us.");
    ...
    protected function validate($data)
    {
        ...
        if ((strlen($data['email']) > 96) || !filter_var($data['email'], FILTER_VALIDATE_EMAIL)) {
             ...
             * Return false
             **/
            $this->session->data['message'] = array('alert' => 'error', 'value' => "Enter Valid Email Address");
            return false;
             ...
             * Return false
             **/
            $this->session->data['message'] = array('alert' => 'error', 'value' => "Password length must be 6 digits");
        ...
        if ($data['role'] == null) {
             ...
             * Return false
             **/
            $this->session->data['message'] = array('alert' => 'error', 'value' => "Please Select Role.");
