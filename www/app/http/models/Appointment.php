...
class Appointment extends Model
{
	...
	{
		$query = $this->database->query("INSERT INTO `" . DB_PREFIX . "appointments` (`name`, `email`, `mobile`, `date`, `time`, `slot`, `message`,  `department_id`, `status`, `consultation_type`, `doctor_id`, `patient_id`, `date_of_joining`, `appointment_id`, `optician_name`, `optician_email`, `optician_address`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ", array($this->database->escape($data['name']), $this->database->escape($data['mail']), $this->database->escape($data['mobile']), $this->database->escape($data['date']), $this->database->escape($data['time']), $this->database->escape($data['slot']), $data['message'], $this->database->escape($data['department']), '1',  $this->database->escape($data['consultation_type']), $this->database->escape($data['doctor']), $this->database->escape($data['patient_id']), $data['datetime'], $data['appointment_id'], $data['optician_name'], $data['optician_email'], $data['optician_address']));
		if ($query->num_rows > 0) {
