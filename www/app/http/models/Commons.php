...
class Commons extends Model
{
    ...
    {
        $query = $this->database->query("Select f.*,a.is_glaucoma_required From `" . DB_PREFIX . "followup_appointment` as f JOIN `" . DB_PREFIX . "appointments` AS a ON f.appointment_id = a.id WHERE f.followup_status ='".constant('STATUS_FOLLOWUP_NEW')."' AND f.reminder_count = '0' ");
