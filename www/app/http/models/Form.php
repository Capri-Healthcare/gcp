...
class Form extends Model
{
	...
	{
		$query = $this->database->query("SELECT f.id, f.name, f.description, GROUP_CONCAT(DISTINCT(d.name)) AS departments, (SELECT COUNT(*) AS tot FROM " . DB_PREFIX . "form_field AS ff WHERE ff.form_id = f.id AND ff.is_active = 'Y') AS no_of_questions, f.status, f.created_at 
	...
	public function getForms($ids)
	{
		if(empty($ids)){
			...
			$query = $this->database->query("SELECT * FROM `" . DB_PREFIX . "form` WHERE `id` IN (".implode(",", $ids).") ");
	...
	{
		$query = $this->database->query("UPDATE `" . DB_PREFIX . "form` SET `name` = ?, `description` = ?, `status` = ?, `updated_at` = ?, `updated_by` = ? WHERE `id` = ?", array($this->database->escape($data['name']), $this->database->escape($data['description']), $data['status'], date('Y-m-d H:i:s'), (int)$data['user_id'], (int)$data['id']));
	...
	{
		$query = $this->database->query("INSERT INTO `" . DB_PREFIX . "form` (`name`, `description`, `status`, `created_at`, `created_by`) VALUES (?, ?, ?, ?, ?)", array($this->database->escape($data['name']), $this->database->escape($data['description']), $data['status'], date('Y-m-d H:i:s'), (int)$data['user_id']));
	...
	public function getPreConsultationFormsByDepartments($department_id)
	{
		...
		WHERE fd.department_id = ? AND fd.is_active = 'Y' AND fd.form_id = f.id AND f.status = 'Y' ", array((int)$department_id));
		if ($query->num_rows > 0) {
			return $query->rows;
		} else {
			return false;
