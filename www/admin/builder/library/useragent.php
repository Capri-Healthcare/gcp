...
class Useragent {
	...
	public function __construct($registry)
	{
		...
		if (isset($this->session->data['user_id']) && isset($this->session->data['login_token'])) {
			if ($this->validateLoginToken($this->session->data['login_token'])) {
				...
				$query = $this->database->query("SELECT u.user_id, u.email, u.firstname, u.lastname, u.mobile, u.picture, u.address,u.optician_shop_name,ur.id AS role_id, ur.name AS role, ur.permission, d.id AS doctor FROM `" . DB_PREFIX . "users` AS u LEFT JOIN `" . DB_PREFIX . "user_role` AS ur ON ur.id = u.user_role LEFT JOIN `" . DB_PREFIX . "doctors` AS d ON d.user_id = u.user_id WHERE u.user_id = ? AND u.status = ?", array((int)$this->session->data['user_id'], '1'));
				if ($query->num_rows > 0) {
					$this->user = $query->row;
	...
	public function hasPermission($data)
	{
		...
		if($data == 'invoice/sendpreviewinvoice') $data = "invoice/sendinvoice";
