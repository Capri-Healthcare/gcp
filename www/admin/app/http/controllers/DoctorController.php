...
class DoctorController extends Controller
{
	...
	**/
	public function indexEdit()
	{
		...
		**/
		...
		$data['doctor_address'] = $this->model_doctor->getDoctorAddressHtml($id);
	...
	**/
	public function indexAction()
	{
		...
		**/
		...
		if($data['form_type'] == 'doctor-web') {
			...
			$this->model_doctor->updateDoctorWebSiteData($data['doctor']);
			$this->session->data['message'] = array('alert' => 'success', 'value' => 'Doctor updated successfully.');
			$this->url->redirect('doctor/edit&id='.$data['doctor']['id']);
		}
		if($data['form_type'] == 'doctor-appointment') {
			$data['doctor']['time'] = json_encode($data['doctor']['time']);

			$this->model_doctor->updateDoctorAppointmentInfo($data['doctor']);
			$this->session->data['message'] = array('alert' => 'success', 'value' => 'Doctor updated successfully.');
			$this->url->redirect('doctor/edit&id='.$data['doctor']['id']);
		}
		if($data['form_type'] == 'doctor-holidays') {
			$data['doctor']['weekly'] = json_encode($weekly_holiday);
			$data['doctor']['national'] = json_encode($data['doctor']['national']);

			$this->model_doctor->updateDoctorHollydays($data['doctor']);
			$this->session->data['message'] = array('alert' => 'success', 'value' => 'Doctor updated successfully.');
			$this->url->redirect('doctor/edit&id='.$data['doctor']['id']);
		}
	}
	...
	public function updateAddress(){
		...
		$this->load->model('doctor');
		$this->model_doctor->updateDoctorAddress($data['doctor']);

		$doctor_address = $this->model_doctor->getDoctorAddressHtml($data['doctor']['id']);
		echo $doctor_address;
	}
