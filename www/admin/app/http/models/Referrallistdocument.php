...
class Referrallistdocument extends Model
{
	public function createReferralListDocument($data)
	{
        ...
        {
            $query = $this->database->query("INSERT INTO `" . DB_PREFIX . "referral_list_document` (`referral_list_id`, `followup_id`,`name`, `filename`,`created_at`, `created_by`) VALUES (?,?,?,?,?,?) ", array('0', $this->database->escape($data['followup_id']), $this->database->escape($data['name']), $this->database->escape($data['file_name']), date('Y-m-d H:i:s'), (int)$data['user_id']));
