...
class Opticianreferral extends Model
{
    ...
    {
        $query = $this->database->query("INSERT INTO `" . DB_PREFIX . "referral_list` (`first_name`, `last_name`,`mobile`,`landline_number`,`office_phone`,`email`,`gender`, `dob`, `address1`, `address2`, `city`, `zip_code`,`created_by`,`created_at`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", array($this->database->escape($data['first_name']), $data['last_name'], $this->database->escape($data['mobile']),$this->database->escape($data['office_phone']),$this->database->escape($data['office_phone']), $this->database->escape($data['email']), $this->database->escape($data['gender']), date("Y-m-d", strtotime($data['dob'])), $this->database->escape($data['address_1']), $this->database->escape($data['address_2']), $this->database->escape($data['city']), $this->database->escape($data['zip_code']), $this->database->escape($data['user_id']), date('Y-m-d H:i:s')));
