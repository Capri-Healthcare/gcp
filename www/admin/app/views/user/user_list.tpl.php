...
<div class="page-title">
	<div class="row align-items-center">
		...
		<div class="col-sm-6 text-right">
			...
			<?php if ($page_add) { ?>
				<a href="<?php echo URL_ADMIN.DIR_ROUTE.'user/add'; ?>" class="btn btn-primary btn-sm"><i class="ti-plus pr-2"></i> New User</a>
			<?php } ?>
		</div>
	</div>
</div>
				...
				<tbody>
					<?php if (!empty($result)) { foreach ($result as $key => $value) { ?>
						<tr>
							...
							<td>
								<p class="text-primary m-0"><?php echo $value['firstname'] .' '. $value['lastname']; ?></p>
							...
							<?php if ($page_edit || $page_delete) { ?>
								<td class="table-action">
									<?php if ($value['user_role'] != "1") { if ($page_edit) { ?>
										<a href="<?php echo URL_ADMIN.DIR_ROUTE.'user/edit&id='.$value['user_id'];?>" class="text-primary edit" data-toggle="tooltip" title="Edit"><i class="ti-pencil-alt"></i></a>
									<?php } if ($page_delete) { ?>
