<?php include (DIR_ADMIN.'app/views/common/header.tpl.php'); ?>
<div class="page-title">
	<div class="row align-items-center">
		<div class="col-sm-6">
			<h2 class="page-title-text d-inline-block"><?php echo $page_title; ?></h2>
			<div class="breadcrumbs d-inline-block">
				<ul>
					<li><a href="<?php echo URL_ADMIN; ?>">Dashboard</a></li>
					<li><?php echo $page_title; ?></li>
				</ul>
			</div>
		</div>
		<div class="col-sm-6 text-right">
			<div class="dropdown d-inline-block m-2">
				<a class="btn btn-white btn-sm dropdown-toggle" data-toggle="dropdown"><i class="ti-download text-primary pr-2"></i> Export</a>
				<ul class="dropdown-menu dropdown-menu-right export-button">
					<li><a class="pdf"><i class="far fa-file-pdf pr-2"></i>PDF</a></li>
					<li><a class="excel"><i class="far fa-file-excel pr-2"></i>Excel</a></li>
					<li><a class="csv"><i class="ti-clipboard pr-2"></i>CSV</a></li>
					<li><a class="print"><i class="ti-printer pr-2"></i>Print</a></li>
					<li><a class="copy"><i class="ti-layers pr-2"></i>Copy</a></li>
				</ul>
			</div>
			<a href="<?php echo URL_ADMIN.DIR_ROUTE.'role/add'; ?>" class="btn btn-primary btn-sm"><i class="ti-plus pr-2"></i> New User Role</a>
		</div>
	</div>
</div>
<div class="panel panel-default">
	<div class="panel-body">
		<div class="table-responsive">
			<table class="table table-bordered table-striped datatable-table" width="100%">
				<thead>
					<tr class="table-heading">
						<th>#</th>
						<th>Role Name</th>
						<th>Description</th>
						<th>Created Date</th>
						<th></th>
					</tr>
				</thead>
				<tbody>
					<?php if (!empty($result)) { foreach ($result as $key => $value) { ?>
						<tr>
							<td><?php echo $key+1; ?></td>
							<td class="text-primary"><?php echo $value['name'];?></td>
							<td><?php echo $value['description']; ?></td>
							<td><?php echo date_format(date_create($value['date_of_joining']), $common['info']['date_format']); ?></td>
							<td class="table-action">
								<?php if ($value['id'] != "1") { ?>
									<a href="<?php echo URL_ADMIN.DIR_ROUTE.'role/edit&id='.$value['id'];?>" class="text-primary edit" data-toggle="tooltip" title="Edit"><i class="ti-pencil-alt"></i></a>
								<?php } if ((int)$value['id'] > 6) { ?>
									<a class="table-delete text-danger delete" data-toggle="tooltip" title="Delete">
										<i class="ti-trash"></i><input type="hidden" value="<?php echo $value['id'];?>">
									</a>
								<?php } ?>
							</td>
						</tr>
					<?php } } ?>
				</tbody>
			</table>
		</div>
	</div>
</div>

<?php include DIR_VIEW.'common/delete_modal.tpl.php'; include (DIR_ADMIN.'app/views/common/footer.tpl.php'); ?>