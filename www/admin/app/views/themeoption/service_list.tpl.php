...
<div class="page-title">
	<div class="row align-items-center">
		...
		<div class="col-sm-6 text-right">
			...
			<?php if ($page_add) { ?>
				<a href="<?php echo URL_ADMIN.DIR_ROUTE.'service/add'; ?>" class="btn btn-primary btn-sm"><i class="ti-plus pr-2"></i> New Service</a>
			<?php } ?>
		</div>
	</div>
</div>
				...
				<tbody>
					<?php if (!empty($result)) { foreach ($result as $key => $value) { ?>
						<tr>
							...
							<td class="table-img">
								<?php if (!empty($value['picture'])) { ?><img class="img-thumbnail" src="../public/uploads/<?php echo $value['picture'];?>" alt="Image"><?php } ?>
							</td>
							...
							<?php if ($page_edit || $page_delete) { ?>
								<td class="table-action">
									<?php if ($page_edit) { ?>
										<a href="<?php echo URL_ADMIN.DIR_ROUTE.'service/edit&id='.$value['id'];?>" class="text-primary edit" data-toggle="tooltip" title="Edit"><i class="ti-pencil-alt"></i></a>
									<?php } if ($page_delete) { ?>
