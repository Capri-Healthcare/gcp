...
<div class="page-title">
	<div class="row align-items-center">
		...
		<div class="col-sm-6 text-right">
			...
			<?php if ($page_add) { ?>
				<a href="<?php echo URL_ADMIN.DIR_ROUTE.'medicine/purchase/add'; ?>" class="btn btn-primary btn-sm"><i class="ti-plus pr-2"></i> New Purchase</a>
			<?php } ?>
		</div>
	</div>
</div>
...
<div class="panel panel-default">
	<div class="panel-body">
		<div class="table-responsive">
			<table class="table table-middle table-bordered table-striped purchase-table">
				<thead>
					<tr>
						<th>#</th>
						<th>Supplier Name</th>
								...
								<td class="table-action">
									<?php if ($page_view || $page_edit) { ?>
										<div class="dropdown d-inline-block">
											...
											<ul class="dropdown-menu dropdown-menu-right export-button">
												<?php if ($page_view) { ?>
													<li><a href="<?php echo URL_ADMIN.DIR_ROUTE.'medicine/purchase/view&id='.$value['id'];?>"><i class="ti-layout-media-center-alt pr-2"></i>View</a></li>
												<?php } if ($page_edit) { ?>
													<li><a href="<?php echo URL_ADMIN.DIR_ROUTE.'medicine/purchase/edit&id='.$value['id'];?>"><i class="ti-pencil-alt pr-2"></i>Edit</a></li>
												<?php } if ($page_pdf) { ?>
													<li><a href="<?php echo URL_ADMIN.DIR_ROUTE.'medicine/purchase/pdf&id='.$value['id'];?>" target="_blank"><i class="far fa-file-pdf pr-2"></i>PDF</a></li>	
												<?php } ?>
											</ul>
										</div>
									<?php } if ($page_delete) { ?>
...
<script>
	$(document).ready(function () {
		...
		$('.table-date-range').on('apply.daterangepicker', function(ev, picker) {
			window.location.href = '<?php echo URL_ADMIN.DIR_ROUTE; ?>medicine/purchase'+'&start='+picker.startDate.format('YYYY-MM-DD')+'&end='+picker.endDate.format('YYYY-MM-DD');
		});
